...
const ForgotPassword: React.FC = () => {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
      ...
      try {
        ...
        const response = await fetch('http://127.0.0.78000/user/forgot-password/', {
          method: 'POST',
          headers: {
        ...
        toast.success('If an account with that email exists, we have sent a password reset email.');
                ...
                <div className="flex items-center w-full max-w-md px-6 mx-auto lg:w-2/6">
                    <div className="flex-1">
                        <div className="text-center">
                            <h2 className="text-4xl font-bold text-center text-gray-700 dark:text-white">FORGET PASSWORD</h2>
                        ...
                        <div className="mt-8">
                            <form id="form" onSubmit={handleSubmit} >
                                <div>
                                    ...
                                    <input type="email" name="email" id="email" placeholder="example@example.com" value={email} className="block w-full px-4 py-2 mt-2 text-gray-700 placeholder-gray-400 bg-white border border-gray-200 rounded-md dark:placeholder-gray-600 dark:bg-gray-900 dark:text-gray-300 dark:border-gray-700 focus:border-blue-400 dark:focus:border-blue-400 focus:ring-blue-400 focus:outline-none focus:ring focus:ring-opacity-40" required />
                                </div>
                            ...
                            <p className="mt-6 text-sm text-center text-gray-400">Remembered your password? <NavLink to="/"  className="text-blue-500 focus:outline-none focus:underline hover:underline">Sign in</NavLink>.</p>
                        </div>
                    </div>
                </div>
            </div>
